...
const addListNode: UpdateOperationAction<UpdateOperationAddData> = (taskGroup, data) => {
...
const convertListToTask: UpdateOperationAction<UpdateOperationConvertToTaskData> = (
  ...
  const { tasks, listTasks, finallyTasks } = taskGroup;
...
const convertFinallyListToTask: UpdateOperationAction<UpdateOperationConvertToTaskData> = (
  ...
  const { tasks, finallyTasks, finallyListTasks } = taskGroup;
  const usedNames = getTaskNames([...tasks, ...finallyTasks]);
...
const updateTask: UpdateOperationAction<UpdateOperationUpdateTaskData> = (taskGroup, data) => {
...
const addFinallyListTask: UpdateOperationAction<UpdateOperationFixInvalidTaskListData> = (
  taskGroup,
) => {
  const newTaskName = `finally-list-${getRandomChars(6)}`;
  const { finallyListTasks } = taskGroup;
  return {
    ...taskGroup,
    finallyListTasks: [...finallyListTasks, { name: newTaskName }],
  };
};
...
export const applyChange = (
  ...
  switch (type) {
    ...
    case UpdateOperationType.ADD_FINALLY_LIST_TASK:
      return addFinallyListTask(taskGrouping, data as UpdateOperationFixInvalidTaskListData);
    default:
