...
export const createFinallyNode = (height): NodeCreator<FinallyTaskNodeModelData> =>
  createGenericNode(NodeType.FINALLY_NODE, NODE_WIDTH + FINALLY_NODE_PADDING * 2, height);
export const createBuilderFinallyNode = (height): NodeCreator<FinallyTaskNodeModelData> =>
...
export const connectFinallyTasksToNodes = (
  ...
  const finallyTasks = getFinallyTasks(pipeline, pipelineRun);
  const regularRunAfters = getLastRegularTasks(nodes);
  const name = 'finally-node';
  const finallyGroupNode = createFinallyNode(getFinallyTaskHeight(finallyTasks.length, true))(
    name,
    {
      isFinallyTask: true,
      pipeline,
